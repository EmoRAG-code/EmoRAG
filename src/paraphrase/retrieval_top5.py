...
os.environ["CUDA_VISIBLE_DEVICES"] = "2"  # 指定使用 GPU 5

device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')

model_name_or_path = "models--facebook--contriever/snapshots/2bd46a25019aeea091fd42d1f0fd4801675cf699"
...
def main():
    all_results = []
    for rank in ["rank0", "rank1", "rank2", "rank3", "rank4", "rank5", "rank6", "rank7", "rank8"]:
    ...
    for (qid, qtext), texts_scores in query_dict.items():
        ...
        prefix_count = sum(1 for text, _ in top5 if prefix in text)
